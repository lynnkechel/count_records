...
object CreateMySqlPool {
  ...
  def apply(): GenericObjectPool[MySqlProxy] = {
    ...
    if (this.genericObjectPool == null) {
      this.synchronized {
        // 获取MySQL配置参数
        val jdbcUrl = PropertiesManager.getValue("jdbc.url")
        val jdbcUser = PropertiesManager.getValue("jdbc.user")
        val jdbcPassword = PropertiesManager.getValue("jdbc.password")
        val size = PropertiesManager.getValue("jdbc.datasource.size").toInt
